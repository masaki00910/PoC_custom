...
## 2026-04-30 (T-005 F-05-005 住所回復処理オーケストレータ実装)

### セッション目的
ユーザー指示: 「頭から流したときに今まで作ってきたフローに入るフローを作成。最初の分岐部分の未作成部分は TODO + 空クラスで配線、後ほどすぐ実装に入れる状態にする。」

### F-05-005 元フロー読解
元 PA `65f74109-84fa-f011-8406-002248f17ef0` (1763 行) の主分岐:
```
IF document_value ∈ {"000", "*000"}:
    IF ca_id contains "住所既契約突合チェック":
        既契約突合 (3 段プロンプト) → 該当: 確定 / 該当なし: F-05-006 フォールバック
    ELSE: pass
ELSE:
    IF error_flag:
        IF ca_id contains "住所漢字・カナ突合チェック": F-05-006 直行
        ELSE: pass
    ELSE:
        IF ca_id contains "文字数超過チェック": F-05-007 (`92c9430a-...`)
        ELSE: pass
属性情報 Switch (document_item):
    "02_policyholder_postal_code" → 契約者
    "02_insured_postal_code"      → 被保険者
```

副流: F-05-009 (中間結果登録 = 監査永続化) と F-05-003 (点検結果登録) はオーケストレータの主流ではなく途中処理 → PoC スコープ外。

### 実装方針
- **`application/` レイヤー新設**: ADR-0001 で謳ったレイヤー構成に最初の実体投入。`CheckRule.execute() -> CheckResult` の単一返却契約から脱却し、`RunAddressRecovery.execute(input) -> list[CheckResult]` のユースケースとして実装
- **未実装ブランチは Stub**: `NotImplementedError` ではなく「`status=NG` + `details.todo_task=T-XXX`」の `CheckResult` を返す。エラーで落ちず、オーケストレータがフォールバック判定できる
- **属性情報の Switch も移植**: `_resolve_attribute(document_item)` で `DocumentAttribute` enum に変換。case に該当しない場合は元 PA の default (空アクション) と整合する形で空配列を返す

### 変更内容
- 新規:
  - `app/application/__init__.py`
  - `app/application/run_address_recovery.py` — `RunAddressRecovery` ユースケース、`AddressRecoveryInput` Pydantic モデル、`DocumentAttribute` enum、`_resolve_attribute()` Switch 移植
  - `app/domain/rules/contract/__init__.py` `app/domain/rules/contract/existing_contract_match.py` — `ExistingContractMatchRule` (Stub, T-011)
  - `app/domain/rules/text/__init__.py` `app/domain/rules/text/char_limit_check.py` — `CharLimitCheckRule` (Stub, T-006)
  - `tests/unit/application/test_run_address_recovery.py` (9 件)
  - `tests/unit/rules/contract/test_existing_contract_match.py` (1 件)
  - `tests/unit/rules/text/test_char_limit_check.py` (1 件)
  - `tests/integration/test_api_address_recovery.py` (4 件)
- 変更:
  - `app/api/deps.py` — Stub × 2 + オーケストレータ DI factory 追加
  - `app/api/v1/schemas.py` — `CheckResultDTO` / `AddressRecoveryRequest` / `Response` 追加
  - `app/api/v1/checks.py` — `POST /v1/checks/address-recovery` 追加

### 設計上の判断
- **戻り値が `list[CheckResult]`**: 元 PA の `回復結果` 配列に 1:1 対応。CheckRule (単一返却) から application 層 (複数返却) への昇格は、PoC で複数フローを束ねる本来のオーケストレータ責務に合致
- **Stub は NG + マーカー方式**: `details["stub"] = "true"` + `details["todo_task"] = "T-XXX"` で識別可能。オーケストレータの「既契約突合 NG なら F-05-006 にフォールバック」がそのまま現実の AI 応答と Stub の両方で動作する (status だけで分岐できる)
- **F-05-009 中間結果登録 (= 監査ログ永続化)**: 元 PA は各プロンプト実行後に DB 書き込みするが、PoC 段階では監査ログ基盤未整備のため意図的に省略。CLAUDE.md §7 (AI 説明可能性) で要求されている情報は `CheckResult.details` に既に格納されているため、後続で `AuditLogger` Port を追加すれば配線できる
- **既契約突合 → F-05-006 フォールバック**: 元 PA は「既契約データなし」分岐と「既契約突合 NG」分岐の両方で F-05-006 を呼ぶ → Stub の `status=NG` を「両方を一度に擬似的に表す」と解釈すれば既契約データなし扱いとして自然にフォールバック

### 動作確認
- `pytest -q` → **51 passed in 1.13s** (新規 15 + 既存 34 + skip 2 が pass に転換)
- 境界テスト `test_application_does_not_depend_on_infrastructure` / `test_application_does_not_import_cloud_sdk` が初めて稼働 → 違反なし
- Cloud Run デプロイ・curl 動作確認は次セッション

### 次セッションへの申し送り
1. **T-005 を git commit & push**
2. ユーザーが Cloud Shell で `git pull && make gcb-deploy && make run-deploy`
3. curl で 4 パス確認:
   - 郵便番号 "000" + ca_id="住所既契約突合チェック" → results 2 件 (Stub NG → F-05-006 OK)
   - error_flag=true + ca_id="住所漢字・カナ突合チェック" → results 1 件 (F-05-006 OK)
   - ca_id="文字数超過チェック" → results 1 件 (Stub NG, todo_task=T-006)
   - 未知 document_item → results 空配列
4. **T-006 (F-05-007 文字数超過 Stub 解消)** に進む — 既存 Stub クラスを実体実装に差し替えるだけ
