# テーブル定義書: ca_existing_contract_data（ATLAS既契約データ）

## 基本情報

| 項目 | 値 |
|---|---|
| テーブル論理名 | ca_existing_contract_data |
| テーブル表示名 | ATLAS既契約データ |
| エンティティセット名 | ca_existing_contract_datas |
| 説明 | ATLAS既契約データを格納するテーブル（モック）。AI点検の際に、エラーリストの値と既契約データを突合して回復判定を行うための参照元として使用される。 |

## カラム定義

| No | 論理名 | 表示名 | データ型 | 最大長 | 必須 | 主キー | 備考 |
|---|---|---|---|---|---|---|---|
| 1 | ca_existing_contract_dataid | ATLAS既契約データ | primarykey (GUID) | — | システム必須 | PK | |
...
| テーブル | 役割 |
|---|---|
| ca_existing_contract_data（本テーブル） | 処理前の既契約データ（照合元） |
| ca_registered_data | 補正処理後の登録データ（照合先） |
